...
//
//Adapted to shield all complex logic... 
//QR Code - I think level 4 with a max of up to 40 (simple) characters
//---------------------------------------------------------------------
namespace DXC.Technology.Barcodes
{
    ...
    public static class QrCodeGenerator
    {
        ...
        public static string QrCodeAsBase64(string identity)
        {
            if (!string.IsNullOrEmpty(identity))
                return Convert.ToBase64String(ToQrCodeByteArray(identity, 4));
            return "";
        }

        public static byte[] ToQrCodeByteArray(string code, int scale)
        {
            MemoryStream ms = new MemoryStream();            
            ToQrCodeImageBetterResolutionUntested(code, scale).Save(ms, System.Drawing.Imaging.ImageFormat.Gif);
            return ms.ToArray();
        }
        public static Image ToQrCodeImageOriginal(string code, int scale)
        {
            QRCode lQrCode = new QRCode(new Options(code));
            lQrCode.Make();
            Image lResult = new Bitmap(34, 34);
            using (Graphics lGraphics = Graphics.FromImage(lResult))
            {
                lGraphics.Clear(Color.White);
                for (int row = 0; row < lQrCode.GetModuleCount(); row++)
                {
                    for (int col = 0; col < lQrCode.GetModuleCount(); col++)
                    {
                        if (lQrCode.IsDark(row, col))
                        {
                            lGraphics.FillRectangle(Brushes.Black, row + 1, col + 1, row + 1, col + 1);
                        }
                        else
                        {
                            lGraphics.FillRectangle(Brushes.White, row + 1, col + 1, row + 1, col + 1);
                        }
                    }
                }
            }
            if (scale > 1)
            {
                int lScaledSize = scale * 34;
                Bitmap lScaledResult = new Bitmap(lScaledSize, lScaledSize);
                using (Graphics graphics = Graphics.FromImage(lScaledResult))
                {
                    graphics.DrawImage(lResult, new Rectangle(0, 0, lScaledSize, lScaledSize));
                }
                return lScaledResult;
            }
            else
            {
                return lResult;
        ...
        public static Image ToQrCodeImageBetterResolutionUntested(string code, int scale)
        {
            QRCode lQrCode = new QRCode(new Options(code));
            lQrCode.Make();
            int lScale = scale;
            ...
            if (scale >= 5)
                lScale = 5;
            Image lResult = new Bitmap(34 * lScale, 34 * lScale);
            using (Graphics lGraphics = Graphics.FromImage(lResult))
            {
                lGraphics.Clear(Color.White);
                for (int row = 0; row < lQrCode.GetModuleCount(); row++)
                {
                    for (int col = 0; col < lQrCode.GetModuleCount(); col++)
                    {
                        if (lQrCode.IsDark(row, col))
                        {
                            lGraphics.FillRectangle(Brushes.Black, lScale * row + 1, lScale * col + 1, lScale + 1, lScale + 1);
                        }
                        else
                        {
                            lGraphics.FillRectangle(Brushes.White, lScale * row + 1, lScale * col + 1, lScale + 1, lScale + 1);
                        }
                    }
                }
            }
        ...
        private enum QRErrorCorrectLevel : int
        {
            L = 1,
            M = 0,
            Q = 3,
        ...
        private enum QRMaskPattern : int
        {
            PATTERN000 = 0,
            PATTERN001 = 1,
            PATTERN010 = 2,
        ...
        private static class QRUtil
        {
            ...
            internal static int GetLengthInBits(QRMode mode, int type)
            {
                ...
                if (1 <= type && type < 10)
                {
                    ...
                    switch (mode)
                    {
                        ...
                        default:
                            throw new DXC.Technology.Exceptions.NamedExceptions.ParameterInvalidException("Mode", mode, "QrCode generation failed");
                    }
                ...
                {
                    throw new DXC.Technology.Exceptions.NamedExceptions.ParameterInvalidException("Type", type, "QrCode generation failed");
                }
            }
            ...
            internal static bool GetMask(QRMaskPattern maskPattern, int i, int j)
            {
                ...
                switch (maskPattern)
                {
                    ...
                    case QRMaskPattern.PATTERN100: return (Math.Floor(Convert.ToDecimal(i) / 2) + Math.Floor(Convert.ToDecimal(j) / 3)) % 2 == 0;
                    case QRMaskPattern.PATTERN101: return (i * j) % 2 + (i * j) % 3 == 0;
                    case QRMaskPattern.PATTERN110: return ((i * j) % 2 + (i * j) % 3) % 2 == 0;
                    case QRMaskPattern.PATTERN111: return ((i * j) % 3 + (i + j) % 2) % 2 == 0;

                    default:
                        throw new DXC.Technology.Exceptions.NamedExceptions.ParameterInvalidException("MaskPattern", maskPattern, "QrCode generation failed");
        ...
        private struct QRPolynomial
        {
                ...
                : this()
            {
                if (num == null)
                {
                    throw new DXC.Technology.Exceptions.NamedExceptions.ParameterInvalidException("num", "null!", "QrCode generation failed");
                }
        ...
        private static class QRMath
        {
            ...
            internal static int GLog(int n)
            {
                if (n < 1)
                {
                    throw new DXC.Technology.Exceptions.NamedExceptions.ParameterInvalidException("GLog", n, "<1?");
                }
        ...
        private class DataCache : List<int>
        {
        ...
        private struct QRRSBlock
        {
            ...
            public static List<QRRSBlock> GetRSBlocks(int typeNumber, QRErrorCorrectLevel errorCorrectLevel)
            {
                ...
                if (rsBlock == null)
                {
                    throw new DXC.Technology.Exceptions.NamedExceptions.NoResultFoundException("typeNumber/errorCorrectLevel", typeNumber + "/" + errorCorrectLevel, "QrCode generation failed"); 
                }
        ...
        private class QRCode
        {
            ...
            private void setupTypeNumber(bool test)
            {
                ...
                for (var i = 0; i < 18; i++)
                {
                    ...
                    this.m_modules[(int)Math.Floor(Convert.ToDecimal(i) / 3)][i % 3 + this.m_moduleCount - 8 - 3] = mod;
                }

                for (var i = 0; i < 18; i++)
                {
                    var mod = (!test && ((bits >> i) & 1) == 1);
                    this.m_modules[i % 3 + this.m_moduleCount - 8 - 3][(int)Math.Floor(Convert.ToDecimal(i) / 3)] = mod;
                }
            }
            ...
            private DataCache CreateData(int typeNumber, QRErrorCorrectLevel errorCorrectLevel, List<QR8bitByte> dataList)
            {
                ...
                {
                    throw new DXC.Technology.Exceptions.NamedExceptions.FieldLengthInvalidException(
                        "QrCode", 
                        "Code length in bits",
                        totalDataCount * 8,
        ...
        private class QRBitBuffer
        {
            ...
            {
                int bufIndex = Convert.ToInt32(Math.Floor(Convert.ToDecimal(index) / 8));
            ...
            public void PutBit(bool bit)
            {
                ...
                int bufIndex = (int)Math.Floor(Convert.ToDecimal(this.m_length) / 8);
